...
"""
    ...
    response = anthropic_client.messages.create(
        model="claude-3-haiku-20240307", # Updated model name slightly from user example
        max_tokens=4096, # Reduced slightly from example, adjust if needed
        temperature=1, # Lower temp for more deterministic JSON structure
        messages=[
            {
                "role": "user",
                "content": prompt
...
"""
    ...
    response = anthropic_client.messages.create(
        model="claude-3-sonnet-20240229", # Updated model name slightly from user example
        max_tokens=4096, # Adjusted max_tokens, user example was high (20000)
        temperature=1, # Lower temp for more structured output
        messages=[
            {
                "role": "user",
                "content": prompt
...
def get_analysis_results(text_id: str, content: str) -> Tuple[List[CharacterDetail], List[NarrativeElement]]:
    ...
    log_entry = APILogger.log_api_request(
        ...
        request_data={"text_id": text_id, "content_length": len(content)},
        text_id=text_id
    )
    ...
    try:
        ...
        APILogger.log_api_response(
            log_entry=log_entry,
            response_data={"characters_count": len(characters)},
            status="success"
        )
    except Exception as e:
        APILogger.log_api_response(log_entry=log_entry, response_data={"error": str(e)}, status="error")
    ...
    log_entry_p2 = APILogger.log_api_request(
        operation="text_analysis_phase2_segmentation",
        request_data={"text_id": text_id, "characters_count": len(characters)},
        text_id=text_id
    )
    try:
        narrative_elements = analyze_text_phase2_segmentation(content, characters)
        APILogger.log_api_response(
            log_entry=log_entry_p2,
            response_data={"segments_count": len(narrative_elements)},
            status="success"
        )
    except Exception as e:
        APILogger.log_api_response(log_entry=log_entry_p2, response_data={"error": str(e)}, status="error")
...
def process_text_analysis(db: Session, text_id: str, content: str) -> models.Text:
    ...
    for char_detail in characters_data:
        db_character = crud.create_character(
            ...
            persona_description=char_detail.get("persona_description"),
            intro_text=char_detail.get("intro_text")
            # Add provider_id=None if needed by CRUD signature
        )
    ...
    for i, element in enumerate(narrative_elements):
        ...
        if db_character:
            db_segment = crud.create_text_segment(
                db=db,
                text_id=db_text.id, # Use the UUID from db_text
                character_id=db_character.id, # Use the UUID from mapped character
                text=element.get("text", ""), # Use renamed field
                sequence=i + 1, # Generate sequence number
                description=element.get("description"),
                speed=element.get("speed"),
                trailing_silence=element.get("trailing_silence")
                # Add audio_file=None if needed by CRUD signature
            )
            db_segments.append(db_segment)
        else:
            # Log or handle cases where a role in phase 2 doesn't match a character from phase 1
            print(f"Warning: Role '{character_name}' found in segmentation but not in character list for text {text_id}. Skipping segment.")
            APILogger.log_api_request(
                operation="segment_creation_warning",
                request_data={"details": f"Role '{character_name}' not found in character map."},
                text_id=str(db_text.id)
            )
    
    print(f"Processed text {text_id}: Created {len(db_characters)} characters and {len(db_segments)} segments.")
    
    # Refresh the text object to potentially load relationships if needed by caller
    # db.refresh(db_text)
    return db_text # Return the updated text object